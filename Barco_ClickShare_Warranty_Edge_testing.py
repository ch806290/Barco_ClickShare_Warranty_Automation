...
class Test(unittest.TestCase):
    ...
    def test_B003_FAST_Edge_Barco_clickshare_warrenty_input_valid(self):
        """
        [FAST]使用Edge,前往Barco,切換成中文,並輸入合法 value "1863552437",並檢查結果
        """
    ...
    def test_B004_FAST_Edge_Barco_clickshare_warrenty_input_invalid(self):
        """
        [FAST]使用Edge,前往Barco,切換成中文,並輸入不合法 value "123456",並檢查結果
        """
...
if __name__ == '__main__':
    ...
    test_suite = unittest.defaultTestLoader.discover(
        basedir, pattern='Barco_ClickShare_Warranty_Edge_testing.py')

    # 測試結果加入到 BeautifulReport 套件內
    result = BeautifulReport(test_suite)

    # 結果產生Report 檔案名稱為 filename, 敘述為 description, log_path 預設放在跟目錄底下就行
    result.report(filename='Barco_assignment_report_05',
